...
export default function App() {
  ...
  useEffect(() => { supabase.from("catalogue_published_resources").select("*").order("created_at", { ascending: false }).then(({ data }) => { const rows = data || []; const mapped = rows.map((row: any) => ({ slug: row.id, title: row.title, subject: row.subject, exam: row.examination, type: row.resource_type_label, year: row.year ? String(row.year) : "", difficulty: row.level || "", description: row.description || [row.country, row.language].filter(Boolean).join(" · ") || "Published Collifi resource.", country: row.country, url: row.resource_url } as Resource)); setLiveResources(mapped); subjects = Array.from(new Map(rows.map((row: any) => [row.subject_slug, { slug: row.subject_slug, name: row.subject, eyebrow: row.country || "CATALOGUE", description: `${row.examination} · ${row.level}`, accent: "#d9e4f5", icon: BookOpen }])).values()) as Subject[]; resourceTypes = ["All types", ...Array.from(new Set(rows.map((row: any) => row.resource_type_label)))]; exams = ["All exams", ...Array.from(new Set(rows.map((row: any) => row.examination)))]; }); }, []);
