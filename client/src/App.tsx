...
function routeFor(pathname = window.location.pathname) {
  ...
  if (parts[0] === "paper" && parts[1]) return { page: "paper", slug: parts[1] };
...
function ResourceCard({ resource }: { resource: Resource }) {
  const details = [resource.subject, resource.year, resource.language].filter(Boolean).join(" · ");
  return <a className="resource-card" href={`/paper/${resource.slug}`}><div className="resource-meta"><span>{resource.type}</span><span>{resource.subject}</span></div><h3>{resource.title}</h3><p>{resource.description || "Published academic resource."}</p>{details && <div className="resource-bottom"><span>{details}</span><ChevronRight size={16} /></div>}{resource.tags?.length ? <div className="resource-tags">{resource.tags.slice(0, 3).map((tag) => <span key={tag}>{tag}</span>)}</div> : null}<div className="resource-source"><span>{resource.isDirectory ? "Directory / index" : "Open resource"}{resource.sourceName ? ` · ${resource.sourceName}` : ""}</span><ArrowUpRight size={14} /></div></a>;
}
...
function ResourcesPage({ slug }: { slug: string }) {
  type Filters = { country: string; exam: string; level: string; subject: string; type: string; year: string; language: string };
  const query = new URLSearchParams(window.location.search);
  const initial = { country: query.get("country") || "", exam: query.get("exam") || "", level: query.get("level") || "", subject: query.get("subject") || "", type: query.get("type") || "", year: query.get("year") || "", language: query.get("language") || "" };
  const [filters, setFilters] = useState<Filters>(initial);
  const [searchInput, setSearchInput] = useState(slug ? resources.find((item) => item.slug === slug)?.title || "" : query.get("search") || "");
  ...
  const matches = (resource: Resource, ignored?: keyof Filters) => { const checks: Record<keyof Filters, boolean> = { country: !filters.country || resource.country === filters.country, exam: !filters.exam || resource.exam === filters.exam, level: !filters.level || resource.level === filters.level, subject: !filters.subject || resource.subject === filters.subject, type: !filters.type || resource.type === filters.type, year: !filters.year || resource.year === filters.year, language: !filters.language || resource.language === filters.language }; return (Object.keys(checks) as (keyof Filters)[]).every((key) => key === ignored || checks[key]); };
  const values = (key: keyof Filters) => Array.from(new Set(resources.filter((resource) => matches(resource, key)).map((resource) => resource[key]).filter(Boolean) as string[])).sort((a, b) => a.localeCompare(b, undefined, { numeric: true }));
  const options = { country: values("country"), exam: values("exam"), level: values("level"), subject: values("subject"), type: values("type"), year: values("year"), language: values("language") };
  const filtered = useMemo(() => resources.filter((resource) => matches(resource) && [resource.title, resource.subject, resource.type, resource.year, resource.language, resource.description, resource.country, resource.exam, resource.level, ...(resource.tags || [])].join(" ").toLowerCase().includes(search.toLowerCase())), [filters, search, resources.length]);
  const active = (Object.entries(filters) as [keyof Filters, string][]).filter(([, value]) => value);
  const clearAll = () => setFilters({ country: "", exam: "", level: "", subject: "", type: "", year: "", language: "" });
  const select = (key: keyof Filters, label: string, items: string[]) => <label><span>{label}</span><select value={filters[key]} onChange={(event) => setFilter(key, event.target.value)}><option value="">All {label.toLowerCase()}s</option>{items.map((item) => <option key={item} value={item}>{item}</option>)}</select></label>;
  return <PageShell><div className="page-intro"><span className="eyebrow">THE RESOURCE LIBRARY</span><h1>Study with a<br /><em>clear next step.</em></h1><p>Find actual published papers, notes, tests, and study materials from the live Collifi catalogue.</p></div><div className="resource-toolbar"><label className="resource-search"><Search size={17} /><input value={searchInput} onChange={(event) => setSearchInput(event.target.value)} placeholder="Search titles, subjects, tags..." /></label>{select("country", "Country", options.country)}{select("exam", "Examination", options.exam)}{select("level", "Level", options.level)}{select("subject", "Subject", options.subject)}{select("type", "Resource type", options.type)}{select("year", "Year", options.year)}{select("language", "Language", options.language)}</div>{active.length ? <div className="active-filters"><span>Filters</span>{active.map(([key, value]) => <button key={key} onClick={() => setFilter(key, "")}>{value} <X size={13} /></button>)}<button className="clear-filters" onClick={clearAll}>Clear all</button></div> : null}<div className="library-result-line"><span><strong>{filtered.length}</strong> resources available</span><span>{active.length ? `${active.length} active filter${active.length === 1 ? "" : "s"}` : "Published catalogue resources"}</span></div>{filtered.length ? <div className="resource-grid">{filtered.map((resource) => <ResourceCard key={resource.slug} resource={resource} />)}</div> : <ComingSoon title="No published resources match these filters." copy="Try removing a filter or clearing the search. New published resources will appear automatically from Supabase." action={<button className="text-button" onClick={clearAll}>Clear filters <ArrowUpRight size={16} /></button>} />}</PageShell>;
}
function PaperPage({ slug }: { slug: string }) {
  const resource = resources.find((item) => item.slug === slug);
  if (!resource) return <PageShell><ComingSoon title="Resource not found." copy="This published resource is no longer available in the live catalogue." action={<button className="text-button" onClick={() => go("/resources")}>Back to resources <ArrowUpRight size={16} /></button>} /></PageShell>;
  return <PageShell><div className="page-intro"><span className="eyebrow">PUBLISHED RESOURCE</span><h1>{resource.title}</h1><p>{resource.description}</p></div><div className="paper-detail"><div className="resource-meta"><span>{resource.type}</span><span>{resource.subject}</span></div><div className="paper-facts"><span>{resource.exam}</span>{resource.level && <span>{resource.level}</span>}{resource.year && <span>{resource.year}</span>}{resource.language && <span>{resource.language}</span>}</div>{resource.tags?.length ? <div className="resource-tags">{resource.tags.map((tag) => <span key={tag}>{tag}</span>)}</div> : null}{resource.sourceName && <p className="paper-source">Source: {resource.sourceName}</p>}<a className="button button-dark" href={resource.url} target="_blank" rel="noreferrer">Open resource <ArrowUpRight size={16} /></a></div></PageShell>;
}
...
function SearchDialog({ onClose }: { onClose: () => void }) {
  ...
  const results = query.trim() ? [...subjects.map((item) => ({ title: item.name, detail: "Subject", path: `/subjects/${item.slug}` })), ...resources.map((item) => ({ title: item.title, detail: `${item.subject} · ${item.type}`, path: `/paper/${item.slug}` })), ...articles.map((item) => ({ title: item.title, detail: `Article · ${item.category}`, path: `/articles/${item.slug}` }))].filter((item) => `${item.title} ${item.detail}`.toLowerCase().includes(query.toLowerCase())).slice(0, 7) : [];
...
export default function App() {
  ...
  useEffect(() => { Promise.all([supabase.from("catalogue_published_resources").select("*").order("created_at", { ascending: false }), supabase.from("catalogue_subject_directory").select("*").order("country").order("name"), supabase.from("catalogue_exam_directory").select("*").order("country").order("name"), supabase.from("catalogue_type_directory").select("*").order("label")]).then(([resourceResult, subjectResult, examResult, typeResult]) => { const rows = resourceResult.data || []; const mapped = rows.map((row: any) => ({ slug: row.slug, title: row.title, subject: row.subject, exam: row.examination, type: row.resource_type_label, year: row.year ? String(row.year) : "", difficulty: row.level || "", description: row.description || [row.country, row.language].filter(Boolean).join(" · ") || "Published academic resource.", country: row.country, level: row.level, language: row.language, tags: row.tags || [], sourceName: row.source_name, sourceUrl: row.source_url, url: row.resource_url, isDirectory: Boolean(row.source_url && row.resource_url === row.source_url) } as Resource)); setLiveResources(mapped); subjects = (subjectResult.data || []).map((row: any) => ({ slug: row.slug, name: row.name, eyebrow: `${row.country} / ${row.examination}`, description: `${row.examination} · ${row.level}`, accent: "#d9e4f5", icon: BookOpen })) as Subject[]; resourceTypes = ["All types", ...(typeResult.data || []).map((row: any) => row.label)]; exams = ["All exams", ...(examResult.data || []).map((row: any) => row.name)]; }); }, []);
  ...
  useEffect(() => {
    const resource = route.page === "paper" ? liveResources.find((item) => item.slug === route.slug) : undefined;
    const title = resource ? `${resource.year ? `${resource.year} ` : ""}${resource.title} | Collifi` : route.page === "home" ? "Collifi — Learn better. Understand more." : `${route.page[0].toUpperCase()}${route.page.slice(1)} — Collifi`;
    const description = resource ? `Access ${resource.title} through Collifi and open the verified original source.` : "Collifi — a searchable learning resource index.";
    document.title = title;
    const setMeta = (name: string, content: string, property = false) => { const attr = property ? "property" : "name"; let node = document.head.querySelector(`meta[${attr}="${name}"]`) as HTMLMetaElement | null; if (!node) { node = document.createElement("meta"); node.setAttribute(attr, name); document.head.appendChild(node); } node.content = content; };
    setMeta("description", description); setMeta("og:title", title, true); setMeta("og:description", description, true); setMeta("og:url", `${window.location.origin}${window.location.pathname}`, true);
    let canonical = document.head.querySelector("link[rel=canonical]") as HTMLLinkElement | null; if (!canonical) { canonical = document.createElement("link"); canonical.rel = "canonical"; document.head.appendChild(canonical); } canonical.href = `${window.location.origin}${window.location.pathname}`;
  }, [route, liveResources]);
  useEffect(() => { const zones = [{ id: "collifi-route-ad", zone: "11743539" }, { id: "collifi-route-ad-2", zone: "11743628" }]; if (route.page === "home") { zones.forEach(({ id }) => document.getElementById(id)?.remove()); return; } zones.forEach(({ id, zone }) => { if (!document.getElementById(id)) { const script = document.createElement("script"); script.id = id; script.dataset.zone = zone; script.src = "https://n6wxm.com/vignette.min.js"; document.body.appendChild(script); } }); }, [route]);
  const content = route.page === "home" ? <Home /> : route.page === "subject" ? <SubjectPage slug={route.slug} /> : route.page === "resources" ? <ResourcesPage slug={route.slug} /> : route.page === "paper" ? <PaperPage slug={route.slug} /> : route.page === "article" ? <ArticlePage /> : route.page === "exams" ? <ExamsPage /> : route.page === "admin" ? <AdminPage onPublished={() => window.location.reload()} /> : <SimplePage kind={route.page} />;
